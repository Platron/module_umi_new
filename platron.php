...
class platronPayment extends payment {
	...
	public function process($template = null) {
		...
		$strCallbackUrl = $www . '/emarket/gateway/' . $this->order->getId();
		...
		list($templateString) = emarket::loadTemplates("emarket/payment/platron/".$template, "form_block");
		return emarket::parseTemplate($templateString, $arrFields);
	...
	protected function _http($url) {
		return mb_strpos($url, 'http://') === 0 || mb_strpos($url, 'https://') === 0 ? $url : 'http://' . $url;
	}
};
